...
export function buildContextRoutes(db: Database, authorizer?: Authorization) {
  ...
  router.use(async (req, res, next) => {
    if(authorizer){
      ...
      if(userId){
        const policies = await authorizer.getContextPolicies(userId, { body, params })
        body.policies = policies;
        next()
      }else{
        logger.error("Failed contexts request: Unauthorized empty user")
        res.status(403).send({ error: "Failed contexts request: Unauthorized empty user" })
        return
      }
    }else{
      next()
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      if(!policies.readAllowed){
        ...
        res.status(500).send(JSON.stringify({ error: "Failed to get context: Unauthorized!" }));
        return;
      }
  ...
  router.post(
    "/:id",
    async (
      req: Request<{ id: string }, {}, { key: string; value: string, policies: PolicyResult }>,
      res,
    ) => {
      ...
      try {
        ...
        if(!policies.updateAllowed){
          ...
          res.status(500).send(JSON.stringify({ error: "Failed to update context: Unauthorized!" }));
          return;
        }
  ...
  router.delete("/:id", async (req: Request<{ id: string }>, res) => {
    ...
    try {
      ...
      if(!policies.deleteAllowed){
        ...
        res.status(500).send(JSON.stringify({ error: "Failed to delete context: Unauthorized!" }));
        return;
      }
  ...
  router.post("/", async (req: Request<{}, {}, { context: Context, policies: PolicyResult }>, res) => {
    const { body } = req;
    try {
      const { body: { policies, context } } = req
      if(!policies.createAllowed){
        logger.error("Failed to insert context: Unauthorized!");
        res.status(500).send(JSON.stringify({ error: "Failed to insert context: Unauthorized!" }));
        return;
      }
