...
export function buildAuthMiddleware(database: Database){
  return async function authMiddleware(
    ...
    if(accessToken){
      try {
        const decoded = jwt.verify(accessToken, process.env.TOKEN_KEY!) as jwt.JwtPayload;
      ...
      try {
        ...
        const decoded = jwt.verify(sessionToken, process.env.TOKEN_KEY! + metakey) as jwt.JwtPayload
